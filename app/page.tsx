...
import { Baby, BookOpen, Gamepad2, GraduationCap, HeartHandshake, Instagram, Layers, Mail, Menu, MessageCircle, Phone, Sparkles, Star, Trophy, X } from "lucide-react";
...
const levels: [LucideIcon, string, string, string][] = [
  [Baby, "پکیج استارتر", "0 تا2 سال", "آشنایی نرم و بازی‌محور با صداها و اولین کلمات انگلیسی."],
  [Sparkles, "پکیج مقدماتی", "2 تا 4 سال", "شناخت حروف، رنگ‌ها و واژه‌های روزمره با تصویر و بازی."],
...
export default function Home() {
  ...
  return <main className="siteMain">
    <header><a className="brand" href="#home"><img src="/images/logo.jpg" alt="لوگوی آکادمی زبان میرلو"/><span><b>آکادمی زبان میرلو</b><em>تربیت کودک دوزبانه</em></span></a><nav>{nav.map(([n,h])=><a key={h} href={h}>{n}</a>)}</nav><Button>مشاهده پکیج‌ها</Button><button className="menu" onClick={()=>setOpen(!open)} aria-label="منو">{open?<X/>:<Menu/>}</button>{open&&<div className="mobile">{nav.map(([n,h])=><a onClick={()=>setOpen(false)} key={h} href={h}>{n}</a>)}</div>}</header>
    <section className="hero" id="home"><div className="orb one"/><div className="orb two"/><div className="heroCopy"><span className="chip"><Star/>پکیج‌های کودکان از تولد تا ۱۲ سال</span><h1>{content.heroTitle && <>{content.heroTitle} </>}<strong>{content.heroHighlight}</strong></h1><p>{content.heroDescription}</p><div className="actions"><Button>انتخاب پکیج مناسب</Button><Button href="#teacher">آشنایی با مدرس</Button></div></div><div className="portrait"><img src={content.teacherImage} alt="مدرس زبان انگلیسی کودکان آکادمی زبان میرلو"/><b>ABC</b><span><MessageCircle/> Hello! Let’s learn</span></div></section>
    <section className="section" id="packages"><Title kicker="مسیر یادگیری" title="مسیر یادگیری متناسب با سن کودک" text="کافی است سن فرزندتان را پیدا کنید؛ پکیج مناسب همان‌جاست."/><div className="grid packages">{content.packages.map((item,index)=>{const I=levels[index]?.[0] ?? Baby;return <article key={item.name} className="card"><div className="cardImg"><img src={item.image || content.packageImage} alt={`تصویر ${item.name}`}/><b>{item.age}</b></div><I/><h3>{item.name}</h3><p>{item.description}</p><Button href={`/packages/${index}`}>معرفی و سفارش پکیج</Button></article>})}</div></section>
    <section className="soft section"><Title kicker="مزیت‌ها" title="چرا آکادمی زبان میرلو؟"/><div className="grid benefits">{benefits.map(([Icon,title,text])=>{const I=Icon;return <article key={title}><i><I/></i><h3>{title}</h3><p>{text}</p></article>})}</div></section>
    <section className="section teacher" id="teacher"><div className="teacherPic"><img src={content.teacherImage} alt="مدرس آکادمی زبان میرلو"/></div><div><small>درباره مدرس</small><h2>{content.teacherTitle}</h2><p>{content.teacherDescription}</p><div className="facts"><div><b>طراحی مسیر آموزشی</b><span>از اولین کلمه تا مکالمه روان، مرحله‌به‌مرحله</span></div><div><b>تمرکز بر کودکان از تولد تا ۱۲ سال</b><span>محتوای متناسب با هر بازه سنی</span></div></div><Button href="#contact">بیشتر با من آشنا شوید</Button></div></section>
    <section className="soft section"><Title kicker="پکیج‌های منتخب" title="محبوب‌ترین پکیج‌های آکادمی زبان میرلو" text="بسته‌های فیزیکی کامل، همراه با راهنمای والدین و پشتیبانی مدرس."/><div className="grid featured">{content.packages.slice(0,4).map((item,index)=><article key={item.name} className="feature"><img src={item.image || content.packageImage} alt={`بسته آموزشی ${item.name}`}/><b>{item.age}</b><h3>{item.name}</h3><p>{item.description}</p><strong>استعلام و ثبت سفارش</strong><Button href={`/packages/${index}`}>معرفی و سفارش پکیج</Button></article>)}</div></section>
    ...
    <footer id="contact"><div><a className="brand" href="#home"><img src="/images/logo.jpg" alt="لوگوی آکادمی زبان میرلو"/><span><b>آکادمی زبان میرلو</b><em>تربیت کودک دوزبانه</em></span></a><p>پکیج‌های آموزش زبان انگلیسی کودکان، متناسب با سن و مسیر یادگیری فرزند شما.</p></div><div><b>لینک‌های مهم</b>{nav.map(([n,h])=><a key={h} href={h}>{n}</a>)}</div><div><b>راه‌های ارتباطی</b><a href={`tel:${content.phone}`}><Phone/> {content.phone}</a><a href={`https://instagram.com/${content.instagram}`}><Instagram/> اینستاگرام آکادمی زبان میرلو</a>{content.email && <a href={`mailto:${content.email}`}><Mail/><span dir="ltr">{content.email}</span></a>}<a href={`https://wa.me/${content.whatsapp}`}><MessageCircle/> واتساپ</a></div></footer><div className="copyright">© {new Date().getFullYear()} آکادمی زبان میرلو | تمامی حقوق محفوظ است.</div>
  </main>;
}
